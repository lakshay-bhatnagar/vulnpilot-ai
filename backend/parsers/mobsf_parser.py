...
def parse_mobsf_json(file_obj: BinaryIO | bytes) -> list[VulnerabilityItem]:
    ...
    records = data.get("vulnerabilities", data.get("findings", [])) if isinstance(data, dict) else []
    ...
    for record in records:
        if not isinstance(record, dict):
            continue
        title = str(record.get("title") or record.get("rule") or record.get("name") or "MobSF Finding")
        target = str(record.get("file") or record.get("path") or data.get("app_name", "mobile-application"))
        findings.append(VulnerabilityItem(title=title, tool_source="MobSF", severity=_severity(record.get("severity") or record.get("level")), target_url=target, cwe=str(record["cwe"]) if record.get("cwe") else None, raw_evidence=str(record.get("description") or record.get("details") or "MobSF reported this mobile application finding."), remediation=str(record.get("recommendation") or record.get("solution")) if record.get("recommendation") or record.get("solution") else None))
    return findings
